...
void drawChar(unsigned char x, unsigned char y, unsigned char letter) {
    int a;
    int b;
    for (a=0; a<5; a++) { //pixel length of 1 character
        for (b=0; b<8; b++) { //pixel height of 1 character
            if ((ASCII[letter-0x20][a]>>b) & 0x01) { //l-0x20 for from the first ASCII character onwards
                ssd1306_drawPixel(x+a, y+b, 1); //turn on that pixel of the character
            }
            else {
                ssd1306_drawPixel(x+a, y+b, 0);//turn off that pixel of the character
            }
        }
    }
}

void drawString(char x, char y, char m[32]) {
    int a;
    for (a=0; a<strlen(m); a++) {
        drawChar(x,y,m[a]); //draw each character of the string
        x=x+5;//offset pixels for the next character
    }
}
